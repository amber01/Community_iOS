...
#define scaleToScreenHeight  [UIScreen mainScreen].bounds.size.height/1136.0f
...
#warning mark -- send background view
- (UIView *)blackTranslucentBackgroundView {
    if (!_blackTranslucentBackgroundView) {
        ...
        _blackTranslucentBackgroundView.backgroundColor = [UIColor colorWithRed:241.0/255.0 green:236.0/255.0 blue:236.0/255.0 alpha:0.9];
        UILabel *topTipsView = [[UILabel alloc]initWithFrame:CGRectMake(0, 60 * (scaleToScreenHeight + 1), [UIScreen mainScreen].bounds.size.width, 20)];
        topTipsView.textAlignment = NSTextAlignmentCenter;
        topTipsView.text = @"请选择板块";
        [_blackTranslucentBackgroundView addSubview:topTipsView];
        
        UIImageView *btnImageView = [[UIImageView alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/2-20, [UIScreen mainScreen].bounds.size.height - 45, 40, 40)];
        btnImageView.image = [UIImage imageNamed:@"send_close_btn.png"];
        [_blackTranslucentBackgroundView addSubview:btnImageView];
    }
