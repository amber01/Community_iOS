...
{
    if ((NSNull *)telephone  == [NSNull null]) {
        return @"";
    }
    if ([telephone length]>0) {
        NSString *str = [NSString stringWithFormat:@"%@****%@",[telephone substringToIndex:3],[telephone substringFromIndex:7]];
        return str;
    }
    return @"";
}
...
//app版本判断(返回0,1,2,3分别表示最新版本，低于最新版本但是高于最低版本，低于最低版本，获取版本格式不对)
           ...
           newVer:(NSString *)newVer
{
    ...
    NSArray *nowlist=[nowVer componentsSeparatedByString:@"."];
    NSArray *oldlist=[oldVer componentsSeparatedByString:@"."];
    NSArray *newlist = [newVer componentsSeparatedByString:@"."];
    
    if ([nowlist count]!=3||[oldlist count]!=3) {
        //获取的版本格式不正确
        return 3;
    }
    //是否低于最低版本
    for (int i=0; i<3; i++) {
        if ([[nowlist objectAtIndex:i] intValue]<[[oldlist objectAtIndex:i] intValue]) {
            return 2;
        }else if ([[nowlist objectAtIndex:i] intValue]>[[oldlist objectAtIndex:i] intValue]){
            break;
        }
    }
    //是否为最新版本
    for (int i=0; i<3; i++) {
        if ([[nowlist objectAtIndex:i] intValue]<[[newlist objectAtIndex:i] intValue]) {
            return 1;
        }else if ([[nowlist objectAtIndex:i] intValue]>[[newlist objectAtIndex:i] intValue]){
            break;
        }
    }
    return 0;
