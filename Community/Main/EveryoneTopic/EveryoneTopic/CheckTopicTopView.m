...
@implementation CheckTopicTopView

- (instancetype)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
        UIView *topiBGview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, 50)];
        topiBGview.backgroundColor = [UIColor whiteColor];
        [self addSubview:topiBGview];
        
        self.topicLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 50/2 - 10, ScreenWidth - 30, 20)];
        _topicLabel.textColor = [UIColor blueColor];
        [self addSubview:_topicLabel];
        
        UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, topiBGview.bottom, ScreenWidth, 10)];
