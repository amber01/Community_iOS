...
- (void)layoutBarAndSlide{
     ...
     *  改变滚动条初始化位置
     */
    CGRect frame = tabbar_.trackView_.frame;
    frame.origin.y = self.tabbarHeight - self.tabbarBottomSpacing+1;
    tabbar_.trackView_.frame = frame;
}
...
- (void)buildTabbar:(DLSlideTabbarType)type{
    ...
    for (DLTabedbarItem *item in self.tabbarItems) {
        ...
        if (type == DLSlideTabbarImageRitht) {
            barItem.isImageTop = NO;
        }else if (type == DLSlideTabbarImageTop){
            barItem.isImageTop = YES;
        }
    }
    ...
    tabbar_.tabbarHeight = self.tabbarHeight;
    tabbar_.tabbarBottomSpacing = self.tabbarBottomSpacing;
}
